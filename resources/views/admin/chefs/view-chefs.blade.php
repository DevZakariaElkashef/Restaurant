                            ...
                            <div class="items" id="{{ $chef->id }}">
                                <div class="item-content">
                                    <div class="user-profile">
                                        ...
                                        <img src="{{ asset('img/users/'.$chef->image) }}" class="img-thumbnail" alt="avatar">
                                        <div class="user-meta-info">
                                            <p class="user-name" data-name="{{ $chef->name }}">{{ $chef->name }}</p>
                                            <p class="user-work" data-occupation="{{ $chef->role }}">{{ $chef->role }}</p>
                                        </div>
                                    </div>
                                    <div class="user-email">
                                        <p class="info-title">Email: </p>
                                        <p class="usr-email-addr" data-email="{{ $chef->email }}">{{ $chef->email }}</p>
                                    </div>
                                    <div class="user-location">
                                        <p class="info-title">Location: </p>
                                        <p class="usr-location" data-location="{{ $chef->address }}">{{ $chef->address }}</p>
                                    </div>
                                    <div class="user-phone">
                                        <p class="info-title">Phone: </p>
                                        <p class="usr-ph-no" data-phone="{{ $chef->phone }}">{{ $chef->phone }}</p>
                                    </div>
                                    <div class="action-btn">
                                        <a href="{{ route('chefs.edit', $chef->id) }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit-2 edit"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg>
                                        </a>
                                        <a href="{{ route('chefs.destroy', $chef->id) }}" id="delUser" data-id="{{$chef->id}}" title="Delete">
